"""
EOD (End of Day) VIX Filter Strategy
Execution Time: 15:00 CT (uses 14:45 bar)
VIX: Uses day CLOSE (VIX closes at 15:15 CT - 15 min after trade)
"""
...
# =============================================================================
# CONFIGURATION - EOD (15:00 CT execution)
# =============================================================================
...
# EOD specific: 15:00 execution -> 14:45 bar
TARGET_TIME = time(14, 45)
...
def load_vix_data(path: str) -> pd.DataFrame:
    """Load VIX data and compute ROC using CLOSE prices for EOD"""
    ...
    # EOD: Use CLOSE for ROC calculation
    df['vix_roc'] = df['Close'].pct_change(periods=VIX_ROC_PERIOD)
    return df
...
def compute_vix_roc_regime(target_bars: pd.DataFrame, vix_data: pd.DataFrame) -> np.ndarray:
    """EOD: Use same-day VIX Close for ROC (trade at 15:00 CT, VIX closes at 15:15 CT)"""
...
def compute_vix_filter(target_bars: pd.DataFrame, vix_data: pd.DataFrame) -> tuple:
    """EOD: Use same-day VIX CLOSE for filter (trade at 15:00 CT, VIX closes at 15:15 CT)"""
    ...
    for i, dt in enumerate(target_bars.index):
        bar_date = pd.Timestamp(dt.date())
        if bar_date in vix_close.index:
            val = vix_close.loc[bar_date]
            if not pd.isna(val):
                vix_values[i] = val
...
def main():
    ...
    print("=" * 70)
    print("EOD (END OF DAY) VIX FILTER STRATEGY")
    print(f"Execution Time: {EXECUTION_TIME} (uses {TARGET_TIME} bar)")
    print(f"VIX Source: Day CLOSE (VIX closes at 15:15 CT)")
    print(f"VIX Filter: Only trade when VIX Close < {VIX_FILTER_THRESHOLD}")
    print("=" * 70)
    ...
    # Filter to target time (14:45 bar for 15:00 execution)
    ...
    # Compute VIX filter using VIX Close
    vix_filter, vix_values = compute_vix_filter(target_bars, vix_df)
    trading_days = vix_filter.sum()
    flat_days = (~vix_filter).sum()
    print(f"Trading days (VIX Close < {VIX_FILTER_THRESHOLD}): {trading_days} ({trading_days/len(vix_filter)*100:.1f}%)")
    print(f"Flat days (VIX Close >= {VIX_FILTER_THRESHOLD}): {flat_days} ({flat_days/len(vix_filter)*100:.1f}%)")
    ...
    print("\n" + "=" * 90)
    print(f"EOD RESULTS - COMPARISON: WITH vs WITHOUT VIX FILTER")
    print("=" * 90)
    ...
    equity_df.to_csv(os.path.join(output_dir, "eod_vix_filter_equity.csv"))

    # Plot
    print("\n\nGenerating plots...")
    fig, axes = plt.subplots(4, 1, figsize=(14, 14))

    fig.suptitle(f'EOD Strategy ({EXECUTION_TIME}) - VIX Close Filter', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('VIX Close Level (Red = No Trading)', fontsize=12)
    ax3.legend()
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "eod_vix_filter_chart.png")
