"""
EOD (15:00 CT) - VIX Level Regime Switching
- High Vol: VIX Close >= 20
- Low Vol: VIX Close < 20
- Uses VIX CLOSE directly (closes at 15:15 CT)
"""
...
def load_vix_data(path):
    """Load VIX data - use CLOSE directly for EOD (15:00 CT)"""
...
def precompute_vix_regime(target_bars, vix_data):
    """EOD: Use same-day VIX Close directly"""
    vix_close_series = vix_data['Close'].copy()
    vix_close_series.index = pd.to_datetime(vix_close_series.index.date)
    regime = np.zeros(len(target_bars), dtype=np.bool_)
    for i, dt in enumerate(target_bars.index):
        bar_date = pd.Timestamp(dt.date())
        if bar_date in vix_close_series.index:
            val = vix_close_series.loc[bar_date]
            if not pd.isna(val):
...
def main():
    ...
    print("=" * 70)
    print(f"EOD VIX LEVEL REGIME SWITCHING ({EXECUTION_TIME})")
    print(f"High Vol = VIX Close >= {VIX_THRESHOLD}")
    print(f"VIX Source: Day CLOSE (direct, no MA)")
    print("=" * 70)
