"""
SOD (7:30 CT) - VIX Level Regime Switching with Separate Long Bias
- High Vol: VIX Open >= 20 → Use HIGH_VOL_BIAS
- Low Vol: VIX Open < 20 → Use LOW_VOL_BIAS
- Tests different long bias per regime
"""
...
# Separate long bias values per regime
LOW_VOL_BIAS_VALUES = [2, 3, 4, 5]   # Bullish during calm markets
HIGH_VOL_BIAS_VALUES = [0, 1, 2]     # Neutral/less bullish during volatile markets
...
@njit
def run_backtest_core(prices, indicators, long_biases, levamount, initial_capital, commission, slippage, multiplier):
    """
    Modified backtest core that accepts per-bar long bias array
    """
    n = len(prices)
...
def main():
    ...
    print("=" * 70)
    print(f"SOD VIX REGIME SWITCHING WITH SEPARATE LONG BIAS ({EXECUTION_TIME})")
    print(f"High Vol (VIX >= {VIX_THRESHOLD}): Bias = {HIGH_VOL_BIAS_VALUES}")
    print(f"Low Vol (VIX < {VIX_THRESHOLD}): Bias = {LOW_VOL_BIAS_VALUES}")
    print("=" * 70)
    ...
    # Generate all combinations with separate bias per regime
    all_combos = list(product(LOW_VOL_BIAS_VALUES, HIGH_VOL_BIAS_VALUES, LOW_VOL_COMBOS, HIGH_VOL_COMBOS))
    ...
    for i, (low_vol_bias, high_vol_bias, (low_offset, low_days), (high_offset, high_days)) in enumerate(all_combos):
        ...
        # Combine indicators based on VIX regime
        combined_ind = np.where(high_vol_mask, high_ind, low_ind)

        # Create per-bar long bias array based on VIX regime
        long_biases = np.where(high_vol_mask, high_vol_bias, low_vol_bias).astype(np.float64)

        equity_arr, trade_pnls = run_backtest_core(prices, combined_ind, long_biases,
        ...
        metrics.update({
            ...
            "low_vol_bars": int(low_vol_mask.sum()),
            "high_vol_bars": int(high_vol_mask.sum())
        })
    ...
    output_path = os.path.join(output_dir, "param_sweep_vix_bias_results.csv")
    results_df.to_csv(output_path, index=False)
    print(f"\nSaved to: {output_path}")

    print("\n" + "=" * 100)
    print("TOP 10 VIX REGIME-SWITCHING WITH SEPARATE LONG BIAS")
    print("=" * 100)

    for idx, (_, row) in enumerate(results_df.head(10).iterrows()):
        print(f"\n#{idx+1}: LowVolBias={row['low_vol_bias']}, HighVolBias={row['high_vol_bias']}")
    ...
    # Compare same bias vs different bias
    print("\n" + "=" * 100)
    print("COMPARISON: Same Bias vs Different Bias per Regime")
    print("=" * 100)

    same_bias = results_df[results_df['low_vol_bias'] == results_df['high_vol_bias']]
    diff_bias = results_df[results_df['low_vol_bias'] != results_df['high_vol_bias']]

    print(f"\nSame bias (LowVol=HighVol): Best CAGR = {same_bias['cagr_pct'].max():.2f}%")
    print(f"Diff bias (LowVol>HighVol): Best CAGR = {diff_bias['cagr_pct'].max():.2f}%")
